...
| **D3** Whole gate runs offline, zero org secrets `[all]` **(load-bearing)** | PASS | `.github/workflows/ci.yaml` sets `AGENT_REGISTRY_PROFILE: local`, installs `.[dev]` only (no `[gcp]` extra), and runs ruff + mypy + pytest + `eval/run_eval.py` with no `secrets.` references. |
