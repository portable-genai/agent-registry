...
The profile list, the ephemeral-settings shape and the adapter constructor are IMPORTED from
``tests/test_contract.py`` rather than restated here. Both files must build the same
settings for the two proofs to be about the same thing, and a second copy of a bespoke
constructor is a copy that drifts. This repository keeps its contract suites flat (no
``tests/contract/`` package and no ``tests/__init__.py``), so ``tests`` resolves as an
implicit namespace package from the repo root, which is where the runner starts the probe.
"""
...
def main(profile: str) -> int:
    ...
    from tests.test_contract import _instantiate, _settings
