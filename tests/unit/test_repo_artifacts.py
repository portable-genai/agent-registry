...
proves each line against the tag `pyproject.toml` names and against the other lockfile". Neither
the header nor this file existed here. The locks opened on `uv pip compile`'s own two-line
provenance comment and pinned `hex-service-kit` at a bare 40-character commit, with nothing in
the repository recording that the commit was `v0.0.5`. Every other consumer of the shared kits
carried the map; this one was the last that did not, so `prove-installed-pins.py` had no tag to
compare an installed version against and `fleet-deps.py` had no `scripts/lock.py` to restore a
header through. The reproducible-install claim (practices check D1) rested on a sentence.
...
def test_every_lockfile_pins_each_commons_at_the_same_commit() -> None:
    ...
    An N-way comparison rather than a pair. The pair form reads `_LOCKFILES` and unpacks it into
    exactly two names, so it raises `ValueError: too many values to unpack` the day a third
    lockfile appears beside them: the glob one screen above exists precisely so a new lockfile
    CANNOT escape these checks, and a hardcoded arity underneath it puts the same blind spot
    straight back.

    Disagreement is always a bug, never a choice: both files are compiled from one
    `pyproject.toml`, so the only way two of them pin different commits is that one was not
    recompiled. `hex-service-kit` is in both, and the dev lock is what the offline gate runs
    while the gcp lock is what the container ships.
    """
    by_file = {
        name: _commons_refs((REPO_ROOT / name).read_text(encoding="utf-8")) for name in _LOCKFILES
    }
    packages = sorted({package for pins in by_file.values() for package in pins})
    assert packages, "no lockfile pins any commons; this check is reading the wrong thing"
...
def test_make_lock_regenerates_every_lockfile_the_repo_ships() -> None:
    ...
    Both of this repository's lockfiles were regenerated by somebody typing the uv command by
    hand, which is why they arrived with uv's own provenance comment in place of the header and
    why nothing recorded which release the pinned commit was. A lockfile has to be named
    somewhere `make lock` reaches, or it is regenerated by hand or not at all.
    """
    reachable = _make_lock_recipe() + (REPO_ROOT / "scripts" / "lock.py").read_text(
        encoding="utf-8"
    )
    ...
    assert "scripts/lock.py" in recipe, (
